...
class DuplicatorTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testExecute()
    {
        ...
        $searchCriteria = $this->getMockBuilder(\Magento\Framework\Api\SearchCriteriaInterface::class)
            ...
            ->setMethods(['getItems'])
            ->getMockForAbstractClass();
        $searchCriteria->expects($this->atLeastOnce())->method('getItems')->willReturn([$product]);
        $this->productRepository->expects($this->atLeastOnce())->method('getList')->willReturn($searchCriteria);
        $this->searchCriteriaBuilder->expects($this->atLeastOnce())->method('create')->willReturn($searchCriteria);
        $this->searchCriteriaBuilder->expects($this->atLeastOnce())->method('setFilterGroups')->willReturnSelf();
        $this->searchCriteriaBuilder->expects($this->atLeastOnce())->method('addFilter')->willReturnSelf();
        $this->productManagement->expects($this->atLeastOnce())->method('assignProducts')
            ->with($idDuplicated, [$product]);
