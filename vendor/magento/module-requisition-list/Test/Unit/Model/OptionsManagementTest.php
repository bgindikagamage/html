...
class OptionsManagementTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testAddOption()
    {
        ...
        $item = $this->getMockBuilder(\Magento\RequisitionList\Api\Data\RequisitionListItemInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        $this->requisitionListItemRepository->expects($this->atLeastOnce())->method('get')->willReturn($item);
    ...
    public function testAddOptionWhenOptionIsArray()
    {
        ...
        $optionModel = $this->getMockBuilder(\Magento\RequisitionList\Model\RequisitionListItem\Option::class)
            ->disableOriginalConstructor()
            ->setMethods(['getData', 'setData', 'getCode', 'addData'])
            ->getMock();
        $item = $this->getMockBuilder(\Magento\RequisitionList\Api\Data\RequisitionListItemInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        ...
        $item->expects($this->atLeastOnce())->method('getOptions')->willReturn(json_encode($option));
        $this->serializer->expects($this->atLeastOnce())->method('unserialize')->willReturn($option);
        ...
        $optionModel->expects($this->atLeastOnce())->method('getData')->willReturn(['option_code' => 'option_value']);
        $optionModel->expects($this->atLeastOnce())
            ->method('addData')
            ->with(['option_code' => 'option_value'])
            ->willReturnSelf();
