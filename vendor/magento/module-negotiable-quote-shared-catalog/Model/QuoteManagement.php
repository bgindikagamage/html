...
class QuoteManagement
{
    ...
    /**
     * Delete products by SKUs from existing quotes if products are unassigned from shared catalog.
     *
     * @param array $skus
     ...
     */
    public function deleteItems(array $skus, $customerGroupId, array $stores = [])
    {
        $this->searchCriteriaBuilder->addFilter('customer_group_id', $customerGroupId);
        $this->searchCriteriaBuilder->addFilter('main_table.is_active', 1);
        if ($stores) {
        ...
        $itemCollection->addFieldToFilter('sku', ['in' => $skus]);
        $itemCollection->addFieldToFilter('quote_id', ['in' => $cartIds]);
        $quote = $this->cartFactory->create();
        //if collection don't have quote then getItems throw exception
        $itemCollection->setQuote($quote)->clear();
        /** @var \Magento\Quote\Model\Quote\Item $cartItem */
        foreach ($itemCollection->getItems() as $cartItem) {
