...
class HistoryManagement implements HistoryManagementInterface
{
    ...
    {
        $mergedArray = array_replace_recursive($lastLogData, $newLogData);
    ...
    {
        $lastLogDataExistId = array_filter(array_column($lastLogData, null, 'field_id'), function ($value) {
            return isset($value['field_id']);
        });
        $newLogDataExistId = array_filter(array_column($newLogData, null, 'field_id'), function ($value) {
            return isset($value['field_id']);
        });

        $updatedLogDataExistId = [];
        foreach ($lastLogDataExistId as $fieldId => $lastLog) {
            if (isset($newLogDataExistId[$fieldId])) {
                $updatedLogDataExistId[$fieldId] = $this->mergeLogChild($lastLog, $newLogDataExistId[$fieldId]);
                unset($newLogDataExistId[$fieldId]);
            }
        }

        $lastLogDataNotExistId = array_filter($lastLogData, function ($value) {
            return !isset($value['field_id']);
        });
        $newLogDataNotExistId = array_filter($newLogData, function ($value) {
            return !isset($value['field_id']);
        });

        return array_merge($lastLogDataNotExistId, $newLogDataNotExistId, $newLogDataExistId, $updatedLogDataExistId);
    }
